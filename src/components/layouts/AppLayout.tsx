  ...
  return (
    <SidebarProvider>
      ...
      <div className={`min-h-dvh flex w-full overflow-hidden ${isDemoMode ? "pt-10" : ""}`}>
        ...
        <MobileBottomNav />
        <UpdateBanner />
        <span className="fixed bottom-1 left-1 z-[9999] text-[10px] text-muted-foreground/40 pointer-events-none select-none hidden md:block"><span className="fixed bottom-1 left-1 z-[9999] text-[10px] text-muted-foreground/40 pointer-events-none select-none hidden md:block">Porta v3.1.9</span></span>
      </div>
    </SidebarProvider>
  );
}
